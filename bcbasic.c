...
int64_t do_binop(enum binop_type b_type, int64_t arg1, int64_t arg2) {
    ...
    uint64_t uarg2 = arg2;
    switch (b_type) {
    /* +, -, and * have the standard 2's complement overflow
       behavior. There is no difference in the results of the signed
       versus unsigned versions of the operations, but doing the
       operations as unsigned avoids undefined behavior in the C
       standard. */
    case PLUS: return uarg1 + uarg2;
    case MINUS: return uarg1 - uarg2;
    case TIMES: return uarg1 * uarg2;
    case DIVIDE: {
	if (arg2 == 0)
